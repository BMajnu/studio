...
import { PlusCircle, Trash2, MessageSquare, Loader2, XIcon, PencilIcon, CheckIcon, RefreshCw, GalleryHorizontal, PanelLeftClose, Search, Edit, PanelLeftOpen, Image } from 'lucide-react';
...
export function HistoryPanel({
  ...
  if (isCollapsed) {
    return (
      <div className={cn("flex h-full flex-col items-center py-4 border-r w-[60px]", className)} 
        style={{ 
          backgroundColor: 'hsl(var(--sidebar-background))', 
          borderColor: 'hsl(var(--sidebar-border))'
        }}>
        <div className="flex flex-col items-center gap-6">
          {/* Logo with hover effect to show unfold button - entire area is clickable */}
          <div 
          ...
          >
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <PanelLeftOpen className="h-5 w-5" style={{ color: 'hsl(var(--sidebar-foreground) / 0.8)' }} />
            </div>
            <div className="opacity-100 group-hover:opacity-0 transition-opacity duration-200">
              <DesAInRLogo width={30} height={30} />
            </div>
          </div>
          ...
          <Button variant="ghost" size="icon" className="h-6 w-6 hover:bg-transparent" 
            style={{ color: 'hsl(var(--sidebar-foreground) / 0.7)' }}
            onClick={() => { onToggleCollapse(); setTimeout(() => searchInputRef.current?.focus(), 100); }}>
  ...
  return (
    <div className={cn("flex h-full flex-col border-r", className)}
      ...
      <div className="px-3 py-4">
        <Button variant="default" className="w-full justify-start gap-2 border-0 rounded-full shadow-md btn-glow"
          style={{ 
            background: 'linear-gradient(to right, hsl(var(--sidebar-primary) / 0.2), hsl(var(--sidebar-accent) / 0.2))',
