...
import { MissingKeysModal } from '@/components/ui/MissingKeysModal'; // Changed to named import
import { AVAILABLE_MODELS } from '@/lib/constants';
import { AllApiKeysExhaustedForModelError } from '@/lib/services/google-ai-service'; 
import { useRouter } from 'next/navigation'; // Import useRouter from 'next/navigation' if not already imported
...
export default function ChatPage() {
  ...
  const currentModelIndexRef = useRef(0); // To track the current model being tried
  const currentApiKeyCycleRef = useRef({ userKeyIndex: 0, envKeyTried: false }); // For cycling through API keys

  const router = useRouter(); // Import useRouter from 'next/navigation' if not already imported
  ...
  const handleSendMessage = useCallback(async (
    ...
    if (!isRegenerationCall && !isUserMessageEdit) {
        // Reset API key and model cycling for new user messages
        currentModelIndexRef.current = 0;
        currentApiKeyCycleRef.current = { userKeyIndex: 0, envKeyTried: false };
    }

    const userProfile = profile;
    const availableUserApiKeys = userProfile.geminiApiKeys?.filter(key => key && key.trim() !== '') || [];
    // const envApiKeyFromEnv = process.env.NEXT_PUBLIC_GEMINI_API_KEY; // Client doesn't need to check .env key for this modal

    // Check for API keys and open modal if none are available from the user AND Firebase AI is not being used.
    // GoogleAIService will handle the .env key as a fallback if user keys fail or are absent.
    if (availableUserApiKeys.length === 0 && !userProfile.useFirebaseAI) {
    ...
    setTimeout(async () => {
        ...
        let assistantMessageWasUpdated = false;
        
        const initialModelUserPref = userProfile.selectedGenkitModelId || DEFAULT_MODEL_ID;
        let initialModelIndex = AVAILABLE_MODELS.findIndex(m => m.id === initialModelUserPref);
        if (initialModelIndex === -1) initialModelIndex = 0;
        
        // Ensure currentModelIndexRef is reset for a brand new message send, 
        // but would retain its value if this logic were part of a direct retry of the same message.
        // The reset is currently handled *before* this setTimeout for new messages.
        if (currentModelIndexRef.current >= AVAILABLE_MODELS.length || currentModelIndexRef.current < 0) { // Safety check for index
            currentModelIndexRef.current = initialModelIndex; // Reset to preferred or default if out of bounds
        }

        let modelsAttemptedCount = 0; 

        while (modelsAttemptedCount < AVAILABLE_MODELS.length) {
          // Ensure currentModelIndexRef.current is within bounds for AVAILABLE_MODELS array
          if (currentModelIndexRef.current >= AVAILABLE_MODELS.length) {
'use client';
...
export default function ChatPage() {
  ...
  const handleSendMessage = useCallback(async (
    ...
    setTimeout(async () => {
        ...
        let initialModelIndex = AVAILABLE_MODELS.findIndex(m => m.id === initialModelUserPref);
        if (initialModelIndex === -1) initialModelIndex = 0;
        currentModelIndexRef.current = initialModelIndex; 
        ...
        while (modelsAttemptedCount < AVAILABLE_MODELS.length) {
          ...
          const modelToTry = AVAILABLE_MODELS[currentModelIndexRef.current].id;
          // Use modelsAttemptedCount for the log message as it reflects the number of tries in this cycle
          console.log(`Attempting AI call with model: ${modelToTry} (Attempt ${modelsAttemptedCount + 1} of ${AVAILABLE_MODELS.length} models in this cycle, Overall Index: ${currentModelIndexRef.current})`);
          aiCallError = null; 

          try {
            const baseInputForFlow = { 
              userName: userProfile.name,
              communicationStyleNotes: userProfile.communicationStyleNotes || '',
              userApiKeys: availableUserApiKeys, 
              modelId: modelToTry,
              useAlternativeImpl: userProfile.useAlternativeAiImpl || false,
              useFirebaseAI: userProfile.useFirebaseAI || false,
            ...
            if (isCustom) {
              ...
              const clientMessageForContext = lastClientMessageInHistory?.text || userMessageContent; // Fallback for safety
              
              const customInstructionInput: ProcessCustomInstructionInput = {
                ...baseInputForFlow, // Use the correctly scoped base input
                clientMessage: clientMessageForContext,
                customInstruction: userMessageContent,
                attachedFiles: filesForFlow, 
                chatHistory: chatHistoryForAI,
                // userName, communicationStyleNotes, modelId, useAlternativeImpl, useFirebaseAI, firebaseApp are from baseInputForFlow
                // userApiKeys is also from baseInputForFlow
              };
              const customOutput = await processCustomInstruction(customInstructionInput);
              finalAiResponseContent = [{ type: 'custom', title: customOutput.title, text: customOutput.response }];
            }
            else if (currentActionType === 'processMessage') {
              try {
                const pmInput: ProcessClientMessageInput = {
                  ...baseInputForFlow, clientMessage: userMessageContent,
                ...
                const arInput: AnalyzeClientRequirementsInput = { // Make sure this also uses baseInputForFlow
                  ...baseInputForFlow, clientMessage: userMessageContent,
                  attachedFiles: filesForFlow, chatHistory: chatHistoryForAI
                };
              ...
              const requirementsInput: AnalyzeClientRequirementsInput = { ...baseInputForFlow, clientMessage: userMessageContent, attachedFiles: filesForFlow, chatHistory: chatHistoryForAI };
              ...
              const engagementInput: GenerateEngagementPackInput = {
                ...baseInputForFlow, clientMessage: userMessageContent, designerName: userProfile.name,
              ...
              const ideasInput: GenerateDesignIdeasInput = {
                  ...baseInputForFlow, designInputText: userMessageContent || "general creative designs",
                  attachedFiles: filesForFlow, chatHistory: chatHistoryForAI
              };
              ...
              if (!designFile || !designFile.dataUri) {
                ...
                if(isMounted.current) toast({ title: "Image Required", description: "Please attach a design image to use the 'Check Designs' feature.", variant: "destructive" });
                  ...
                  ...baseInputForFlow, clientPromptOrDescription: userMessageContent || "Client requirements as per conversation history.",
              ...
              const editingInput: GenerateEditingPromptsInput = {
                  ...baseInputForFlow, designToEditDataUri: designToEditDataUriForFlow,
              ...
              const platformInput: GeneratePlatformMessagesInput = {
                ...
                // modelId and userApiKeys are inherited from baseInputForFlow
              };
            ...
            if (!isRegenerationCall && !isUserMessageEdit && !aiCallError) {
              // Reset for next new message, not for retries within the same message.
              // currentApiKeyIndexRef.current = 0; // This was for a single key, GoogleAIService handles cycling now
            }
            // Successful call, break from model cycling loop
            break; 

          } catch (error: any) {
            aiCallError = error; // Store the error
            console.error(`ChatPage: Error with model ${modelToTry} (Action: ${currentActionType}). Error:`, error);

            if (error instanceof AllApiKeysExhaustedForModelError) {
              console.warn(`AllApiKeysExhaustedForModelError for model ${modelToTry}. Cycling to next model.`);
              // This error means all keys (user and .env) failed for this specific model with this specific SDK pathway
              // So, we should cycle to the next model.
              currentModelAttemptIndex = (currentModelAttemptIndex + 1) % AVAILABLE_MODELS.length;
              modelsCycledThrough++;
              
              if (modelsCycledThrough >= AVAILABLE_MODELS.length) {
                // We've tried all models, and all of them exhausted keys.
                const exhaustedAllModelsMsg = `All API keys have been exhausted for all available models. Last attempt was with ${modelToTry}. Please check your API key quotas, add new keys, or try again later.`;
                if (isMounted.current) {
                  toast({ title: "All Models & Keys Exhausted", description: exhaustedAllModelsMsg, variant: "destructive", duration: 7000 });
                  updateMessageById(assistantMessageIdToUse, [{ type: 'text', text: exhaustedAllModelsMsg }], false, true, requestParamsForRegeneration, promptedByMsgIdForNewAssistant);
                }
                break; // Break while loop
              }
              // Continue to the next model in the while loop
            } else {
              // This is a non-key-exhaustion error (e.g., model not found, network issue, specific API error for this model).
              // We might not want to cycle models in this case, or we could, depending on desired behavior.
              // For now, let's assume we should show this error and stop.
              const generalErrorMsg = `An error occurred with model ${modelToTry}: ${error.message || 'Unknown AI error'}. Please try regenerating or select a different model if the problem persists.`;
              if (isMounted.current) {
                toast({ title: "AI Error", description: generalErrorMsg, variant: "destructive", duration: 7000 });
                updateMessageById(assistantMessageIdToUse, [{ type: 'text', text: generalErrorMsg }], false, true, requestParamsForRegeneration, promptedByMsgIdForNewAssistant);
              }
              // Since this isn't a key exhaustion for *this* model, we break the model cycling.
              // The user can choose to regenerate (which might try the same model or others if the selection changed).
              modelsCycledThrough = AVAILABLE_MODELS.length; // Force exit of while loop
              break; 
            }
          } // End of catch block
        } // End of while loop for model cycling

        if (aiCallError && finalAiResponseContent.length === 0 && isMounted.current) {
          // This block executes if the while loop finished (e.g., all models tried and failed with key exhaustion)
          // and no successful response was set.
          let finalErrorText = `Failed to get a response. Last error: ${aiCallError.message || 'Unknown AI error'}`;
          if (aiCallError instanceof AllApiKeysExhaustedForModelError) {
               finalErrorText = `All API keys were exhausted for all attempted models. Last attempt was with ${AVAILABLE_MODELS[currentModelAttemptIndex].id}. Please check your API key quotas or add more keys.`;
          }
          updateMessageById(assistantMessageIdToUse, [{ type: 'text', text: finalErrorText }], false, true, requestParamsForRegeneration, promptedByMsgIdForNewAssistant);
        }
        

        if (isMounted.current) setIsLoading(false);

        if (currentSession && userIdForHistory && isMounted.current && !aiCallError) { 
            setMessages(currentMessagesFromState => {
              ...
              const apiKeyForNameGen = (profile?.geminiApiKeys && profile.geminiApiKeys.length > 0 && profile.geminiApiKeys[0]) 
              ...
              saveSession(
                  ...
                  AVAILABLE_MODELS[currentModelAttemptIndex].id, // Save with the model that (potentially) succeeded or was last tried
                  apiKeyForNameGen
      ...
      lastSelectedActionButton, messagesRef, // messagesRef is important here
      isCustomMessage // Ensure isCustomMessage is a dependency if used directly in this callback
    ]);
  ...
  useEffect(() => {
    if (pendingAiRequestAfterEdit && isMounted.current) {
      ...
      // Reset model/key cycle for new AI request triggered by edit
      currentModelIndexRef.current = 0;
      currentApiKeyCycleRef.current = { userKeyIndex: 0, envKeyTried: false };

      handleSendMessage(
        content,
        lastSelectedActionButton || 'processMessage', // Use last selected action or default
        undefined,
        attachments,
        isEdit,
        false,
  ...
  }, [pendingAiRequestAfterEdit, handleSendMessage, isCustomMessage, lastSelectedActionButton]);
  ...
  const handleRegenerateMessage = useCallback((originalRequestDetailsWithMessageId?: ChatMessage['originalRequest'] & { messageIdToRegenerate: string }) => {
    ...
    // Reset model and key cycle refs for regeneration
    currentModelIndexRef.current = 0; 
    currentApiKeyCycleRef.current = { userKeyIndex: 0, envKeyTried: false };
  ...
  const handleConfirmEditAndResendUserMessage = useCallback((messageId: string, newContent: string, originalAttachments?: AttachedFile[]) => {
    setMessages(prevMessages => {
        ...
        const updatedUserMessage: ChatMessage = {
          ...
          linkedAssistantMessageId: undefined, // Clear link as new assistant message will be generated
        };

        // Remove all messages after the edited one, including its previously linked assistant message
        let messagesUpToEditedAndIncluding = prevMessages.slice(0, messageIndex);
        messagesUpToEditedAndIncluding.push(updatedUserMessage); // Add the updated user message
        
        if (isMounted.current) {
            // Set up for AI to respond to this edited message
            setPendingAiRequestAfterEdit({
                content: newContent,
                attachments: originalAttachments,
                isUserMessageEdit: true,
                editedUserMessageId: updatedUserMessage.id, // Pass the ID of the user message being edited
            });
        }
  ...
  const handleAction = useCallback((action: ActionType) => {
    if (!profile) {
       toast({ title: "Profile Required", description: "Please complete your profile to use AI actions.", variant: "destructive"});
       return;
    }
    ...
    setTimeout(() => setActiveActionButton(null), 500); // Reset visual highlight
    
    // Reset model/key cycle for new action
    currentModelIndexRef.current = 0;
    currentApiKeyCycleRef.current = { userKeyIndex: 0, envKeyTried: false };
  ...
  }, [inputMessage, handleSendMessage, profile, isCustomMessage, toast]);
  ...
  // Define these handlers INSIDE the ChatPage component scope
  const handleGoToProfile = useCallback(() => {
    setIsMissingKeysModalOpen(false);
    router.push('/profile');
  }, [router, setIsMissingKeysModalOpen]);

  const handleOpenTutorial = useCallback(() => {
    setIsMissingKeysModalOpen(false);
    window.open('https://ai.google.dev/gemini-api/docs/api-key', '_blank');
  }, [setIsMissingKeysModalOpen]);
